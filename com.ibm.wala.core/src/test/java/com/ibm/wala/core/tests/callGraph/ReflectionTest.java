...
public class ReflectionTest extends WalaTestCase {
    ...
    public void testReflect4() throws IOException, ClassHierarchyException, IllegalArgumentException, CancelException {
        ...
        MethodReference extraneousMR = MethodReference.findOrCreate(extraneousTR, "<init>",
    ...
    public void testReflect7() throws Exception {
        ...
        MethodReference newInstanceMr = MethodReference.findOrCreate(constrTr, "newInstance",
    ...
    /**
     * Test that when analyzing Reflect12, the call graph does not include a
     * node for reflection.Helper.n but does include a node for
     * reflection.Helper.m
     */
    @Test
    ...
    /**
     * Test that when analyzing Reflect13, the call graph includes both a node
     * for reflection.Helper.n and a node for reflection.Helper.m
     */
    @Test
    ...
    /**
     * Test that when analyzing Reflect14, the call graph does not include a
     * node for reflection.Helper.n but does include a node for
     * reflection.Helper.s
     */
    @Test
    ...
    /**
     * Test that when analyzing Reflect15, the call graph includes a node for
     * the constructor of Helper that takes 2 parameters and for Helper.n, but
     * no node for the constructors of Helper that takes 0 or 1 parameters.
     */
    @Test
    ...
    /**
     * Test that when analyzing Reflect21, the call graph includes a node for
    ...
    /**
     * Test that when analyzing Reflect22, the call graph includes a node for
     * the constructor of {@code Helper} that takes one {@link Integer}
     * parameters. This is to test the support for
     ...
     *
     * <p>
     * Through the pointer analysis in the CallGraph construction process, it
     * can be inferred that the type pointed to by the 0th parameter of the
     * {@code com.ibm.wala.test.People#doNothing()} method is {@code Helper}
     *
     * <p>
     * This is to test the support for Object.getClass().
     */
    @Test
    public void testReflect24() throws WalaException, IllegalArgumentException, CancelException, IOException {
        ...
        IClassHierarchy cha = findOrCreateCHA(scope);
        Iterable<Entrypoint> entrypoints = com.ibm.wala.ipa.callgraph.impl.Util.makeMainEntrypoints(scope, cha,
                TestConstants.REFLECT24_MAIN);

        AnalysisOptions options = CallGraphTestUtil.makeAnalysisOptions(scope, entrypoints);
        Pair<CallGraph, PointerAnalysis<InstanceKey>> pair = CallGraphTestUtil.buildNCFA(1, options, new AnalysisCacheImpl(), cha,
                scope);

        CallGraph cg = pair.fst;
        PointerAnalysis<InstanceKey> pointerAnalysis = pair.snd;

        TypeReference helperTr = TypeReference.findOrCreate(ClassLoaderReference.Application, "Lreflection/Helper");
        IClass helperClass = cha.lookupClass(helperTr);
        Assert.assertNotNull(helperClass);

        TypeReference reflect24Tr = TypeReference.findOrCreate(ClassLoaderReference.Application, "Lreflection/Reflect24");
        MethodReference mr = MethodReference.findOrCreate(reflect24Tr, "doNothing", "(Ljava/lang/Class;)V");
        Set<CGNode> nodes = cg.getNodes(mr);
        Assert.assertTrue(nodes.size() == 1);

        // get the pts corresponding to the 0th parameter of the
        // Reflect24#doNothing() method
    ...
    /**
     * Test that when analyzing GetMethodContext, the call graph must contain
     * exactly one call to each of the following methods:
     *
     * <ul>
    ...
    public void testGetMethodContext() throws WalaException, IllegalArgumentException, CancelException, IOException {
        ...
        Selector sfoo = Selector.make("foo()V"), sbar = Selector.make("bar()V"), sbaz = Selector.make("baz()V");
        MethodReference mafoo = MethodReference.findOrCreate(ta, sfoo), mbfoo = MethodReference.findOrCreate(tb, sfoo),
                mcfoo = MethodReference.findOrCreate(tc, sfoo), mabar = MethodReference.findOrCreate(ta, sbar),
                mbbar = MethodReference.findOrCreate(tb, sbar), mcbar = MethodReference.findOrCreate(tc, sbar),
                mabaz = MethodReference.findOrCreate(ta, sbaz), mbbaz = MethodReference.findOrCreate(tb, sbaz),
